...
def build_blizzard_2012_from_path(hparams, input_dirs, mel_dir, linear_dir, wav_dir, n_jobs=12, tqdm=lambda x: x):
	...
	for input_dir in input_dirs:
		with open(os.path.join(input_dir, 'sentence_index.txt'), encoding='utf-8') as f:
			for line in f:
				parts = line.strip().split('\t')
				if line.startswith('chp') and len(parts) == 8 and parts[7] != '#empty#':
					basename = parts[0]
					...
					# combine SENT_TXT_BOOK(with puntuation but very inaccurate when the CONFIDENCE_VALUE is low) and SENT_TXT_LAB(generated by an ASR system, thus more accurate since the speaker doesn't always strictly follow the text in the book, but w/o punctuation)
					text = '|'.join([re.sub(r'[#@|]', '', parts[5]), re.sub(r'[#@|]', '', parts[7])]) 
