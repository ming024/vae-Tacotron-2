...
hparams = tf.contrib.training.HParams(
	...
	vae_code_usage = 'deep_concat', #Usage of the output code of the VAE. Can be ('concat', 'deep_concat', 'linear_transformation'). In 'concat' mode the encoded vector produced by the VAE encoder is concatenated to the input of the Prenet. In 'deep_concat' mode the code is concatenated to the inputs of the Prenet, the Decoder LSTM, and the Projection networks. In 'linear_transformation' mode the (vae_dim)-dimensional VAE code c is transformed to a (prenet_layers[-1] + attention_dim)-dimensional vector v by a dense layer without activation, afterwards v is used to modify the input of the LSTM decoder x by element-wise multiplication.
