            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                if step % args.summary_interval == 0 or step == 1:
                    ...
                    if hparams.predict_linear:
                        ...
                        audio.save_wav(wav, os.path.join(wav_dir, 'step-{}-train-wave-from-linear.wav'.format(step)), sr=hparams.sample_rate)
                                                      
                        if hparams.GL_on_GPU:
                            wav = sess.run(GLGPU_lin_outputs, feed_dict={GLGPU_lin_inputs: linear_target})
                            wav = audio.inv_preemphasis(wav, hparams.preemphasis, hparams.preemphasize)
                        else:
                            wav = audio.inv_linear_spectrogram(linear_target.T, hparams)
                        audio.save_wav(wav, os.path.join(wav_dir, 'step-{}-real-wave-from-linear.wav'.format(step)), sr=hparams.sample_rate)

                        #Save real and predicted linear-spectrogram plot to disk (control purposes)
                        plot.plot_spectrogram(linear_prediction, os.path.join(plot_dir, 'step-{}-train-linear-spectrogram.png'.format(step)),
                            title='{}, {}, step={}, loss={:.5f}'.format(args.model, time_string(), step, loss), type='Linear',
                            target_spectrogram=linear_target, max_len=target_length, auto_aspect=False)
                    ...
                    audio.save_wav(wav, os.path.join(wav_dir, 'step-{}-train-wave-from-mel.wav'.format(step)), sr=hparams.sample_rate)
                              
                    if hparams.GL_on_GPU:
                        wav = sess.run(GLGPU_mel_outputs, feed_dict={GLGPU_mel_inputs: target})
                        wav = audio.inv_preemphasis(wav, hparams.preemphasis, hparams.preemphasize)
                    else:
                        wav = audio.inv_mel_spectrogram(target.T, hparams)
                    audio.save_wav(wav, os.path.join(wav_dir, 'step-{}-real-wave-from-mel.wav'.format(step)), sr=hparams.sample_rate)

                    #save alignment plot to disk (control purposes)
                    plot.plot_alignment(alignment, os.path.join(plot_dir, 'step-{}-train-align.png'.format(step)),
                        title='{}, {}, step={}, loss={:.5f}'.format(args.model, time_string(), step, loss),
                        max_len=target_length // hparams.outputs_per_step)
                    #save real and predicted mel-spectrogram plot to disk (control purposes)
                    plot.plot_spectrogram(mel_prediction, os.path.join(plot_dir, 'step-{}-train-mel-spectrogram.png'.format(step)),
                        title='{}, {}, step={}, loss={:.5f}'.format(args.model, time_string(), step, loss), type='Mel',
                        target_spectrogram=target, max_len=target_length, auto_aspect=False)
                ...
                if step % args.eval_interval == 0 or step == 1:
                    ...
                    if hparams.predict_linear:
                        ...
                        if hparams.GL_on_GPU:
                            wav = sess.run(GLGPU_lin_outputs, feed_dict={GLGPU_lin_inputs: lin_t})
                            wav = audio.inv_preemphasis(wav, hparams.preemphasis, hparams.preemphasize)
                        else:
                            wav = audio.inv_linear_spectrogram(lin_t.T, hparams)
                        audio.save_wav(wav, os.path.join(eval_wav_dir, 'step-{}-real-wave-from-linear.wav'.format(step)), sr=hparams.sample_rate)
                    ...
                    if hparams.GL_on_GPU:
                        wav = sess.run(GLGPU_mel_outputs, feed_dict={GLGPU_mel_inputs: mel_t})
                        wav = audio.inv_preemphasis(wav, hparams.preemphasis, hparams.preemphasize)
                    else:
                        wav = audio.inv_mel_spectrogram(mel_t.T, hparams)
                    audio.save_wav(wav, os.path.join(eval_wav_dir, 'step-{}-real-wave-from-mel.wav'.format(step)), sr=hparams.sample_rate)
                    ...
                    plot.plot_spectrogram(mel_p, os.path.join(eval_plot_dir, 'step-{}-eval-mel-spectrogram.png'.format(step)),
                        title='{}, {}, step={}, loss={:.5f}'.format(args.model, time_string(), step, eval_loss), type='Mel',
                        target_spectrogram=mel_t, max_len=t_len, auto_aspect=False)

                    if hparams.predict_linear:
                        plot.plot_spectrogram(lin_p, os.path.join(eval_plot_dir, 'step-{}-eval-linear-spectrogram.png'.format(step)),
                            title='{}, {}, step={}, loss={:.5f}'.format(args.model, time_string(), step, eval_loss), type='Linear',
                            target_spectrogram=lin_t, max_len=t_len, auto_aspect=False)
