...
# tacotron_synthesis_batch_size: 1 to 17
# outputs_per_step: 1 to 3
# sample_rate: 22050(LJSpeech) to 44100(Blizzard-2012) to 22050 to 16000(Bizzard-2013)
# num_freq: change with sample_rate
# n_fft: change with sample_rate
# hop_size: change with sample_rate
# win_size: change with sample_rate
#
# New parameters
# use_vae: True
# vae_dim: 16
# vae_filters: [32, 32, 64, 64, 128, 128]
# vae_kernel: (3, 3)
# vae_stride: (2, 2)
# vae_rnn_units: 128
# vae_weight: 1e-3
...
# decoder_concat_type: 'deep_concat'
# tacotron_gradient_accumulation_steps: 4
#
# May be modified
# input_type: raw to mulaw-quantize (WaveNet quantization)
# gin_channels: -1 (single speaker)
...
hparams = tf.contrib.training.HParams(
	...
	preload_spectrogram = False, #To load all spectrograms into memory at the beginning of training/testing, or to load only several batches of spectrograms that will be used immediately into memory
	###########################################################################################################################################
	...
	#Trim params (there parameters are usually correct for any data, but definitely must be tuned for specific speakers)
	...
	vae_dim = 16, #dimension of VAE code
	vae_filters = [32, 32, 64, 64, 128, 128], #number of VAE convolution filters
	vae_kernel = (3, 3), #kernel size of VAE convolution
	vae_stride = (2, 2), #stride of VAE convolution
	vae_rnn_units = 128, #stride of VAE convolution
	vae_log_var_minimum = None, #minimum value for the log variance generated by the VAE network
	encoder_outputs_revision_type = 'add', #Usage of the output code of the VAE. Can be ('add', 'multiply', None). In 'add' mode the VAE code is fed into a tanh-activated dense layer and then added to the encoder outputs, which is also tanh-activated too. In 'multiply' mode element-wise multiplication is used instead of addition. If None, the encoder outputs are directly used to compute attention, without any revision.
	decoder_concat_type = 'deep_concat', #Usage of the output code of the VAE. Can be ('concat', 'deep_concat', 'multiply'). In 'concat' mode the encoded vector produced by the VAE encoder is concatenated to the input of the Prenet. In 'deep_concat' mode the code is concatenated to the inputs of the Prenet, the Decoder LSTM, and the Projection networks. In 'multiply' mode the (vae_dim)-dimensional VAE code is first projected to (prenet_layers[-1] + attention_dim)-dimensional space by a tanh-activated dense layer, then used to modify the input of the LSTM decoder x by element-wise multiplication.
	...
	vae_weight = 1e-3, #The weight of the KL loss of VAE
	...
	tacotron_synthesis_batch_size = 17, #DO NOT MAKE THIS BIGGER THAN 1 IF YOU DIDN'T TRAIN TACOTRON WITH "mask_encoder=True"!!
	...
	wavenet_debug_wavs = ['/groups/ming/tacotron2/LJSpeech/data/audio/audio-LJ001-0008.npy'], #Path to debug audios. Must be multiple of wavenet_num_gpus.
	wavenet_debug_mels = ['/groups/ming/tacotron2/LJSpeech/data/mels/mel-LJ001-0008.npy'], #Path to corresponding mels. Must be of same length and order as wavenet_debug_wavs.
