...
class Synthesizer:
	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, dim=None, scale=None):
		...
		if hparams.use_vae and not self.gta:
			if mel_filenames:
				...
				vae_codes = None
				for i in range(hparams.tacotron_num_gpus):
					vae_dim = hparams.vae_dim
					device_vae_codes = np.zeros((size_per_device, vae_dim), dtype=np.float32)
					if dim is not None and scale is not None:
	...
	def inference(self, mel_filenames):
		hparams = self._hparams

		#Repeat last sample until number of samples is dividable by the number of GPUs (last run scenario)
		while len(mel_filenames) % hparams.tacotron_synthesis_batch_size != 0:
			mel_filenames.append(mel_filenames[-1])

		assert 0 == len(mel_filenames) % hparams.tacotron_num_gpus
		size_per_device = len(mel_filenames) // hparams.tacotron_num_gpus

		np_references = [np.load(mel_filename) for mel_filename in mel_filenames]
		references_lengths = [len(np_reference) for np_reference in np_references]
		split_infos = []
		...
		for i in range(hparams.tacotron_num_gpus):
			...
			device_reference, max_target_len = self._prepare_targets(device_reference, hparams.outputs_per_step)
			reference_seqs = np.concatenate((reference_seqs, device_reference), axis=1) if reference_seqs is not None else device_reference
			split_infos.append([0, max_target_len, 0, 0]) #Not really used but setting it in case for future development maybe?

		feed_dict = {
			self.mel_references: reference_seqs,
                        self.references_lengths: references_lengths,
                        self.split_infos: np.asarray(split_infos, dtype=np.int32)
		}

		mu = self.session.run(self.model.tower_mu, feed_dict=feed_dict)
